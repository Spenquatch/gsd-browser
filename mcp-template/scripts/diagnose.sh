...
for tool in uv poetry pipx mcp-template; do
...
printenv | grep -E 'ANTHROPIC|MCP_TEMPLATE_MODEL|LOG_LEVEL' || echo "(none set)"
...
printf 'ping\n' | "$PY_BIN" -m mcp_template.cli serve --once || true
