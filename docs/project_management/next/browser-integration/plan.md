...
## Purpose
Land the WebEval Agent browser-streaming/dashboard stack inside the reusable MCP template. This work ports CDP streaming, dashboard takeover security, MCP screenshot tooling, and the upgraded `browser-use` automation stack into the template without regressing privacy or install ergonomics.
...
- **Protected assets:** do not mutate `.git`, `mcp-template/.venv`, sockets under `/tmp`, or dashboard credential stores outside the repo (e.g., OS keychains). Install scripts may write to `~/.local/bin` only when explicitly scoped.
...
## Triad Overview
1. **B1 – Streaming Core**: Port CDP streaming + screenshot fallback + screenshot manager & health instrumentation into the template (Python service + CLI toggles).
