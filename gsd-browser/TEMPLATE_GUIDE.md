...
3. **Adjust configuration**: Update `config.py` defaults + `.env.example` to match your service requirements. Ensure `scripts/print-mcp-config.py` and `gsd mcp config` emit the correct snippet.
4. **Customize scripts**: `scripts/run-local.sh`, `diagnose.sh`, `smoke-test.sh`, and `check-mcp-config.sh` can be tuned for dependencies (browser install, GPU, etc.).
5. **Installer tweaks**: Update `tools/install.sh` manifest metadata or extra dependency steps (e.g., OS packages for a system browser).
