...
def test_c2_chatbrowseruse_missing_api_key_fails_fast_with_guidance() -> None:
    ...
    assert "GSD_LLM_PROVIDER=chatbrowseruse" in message
...
def test_c2_chatbrowseruse_invalid_model_fails_fast_with_guidance() -> None:
    ...
    message = str(excinfo.value)
    assert "Invalid GSD_MODEL" in message
...
def test_c2_provider_missing_credentials_fails_fast_with_guidance(
    ...
    settings = load_settings(
        env={
            "GSD_LLM_PROVIDER": provider,
            "GSD_MODEL": "ignored-model",
            required_env_var: "",
        },
        env_file=None,
    )
    ...
    assert f"GSD_LLM_PROVIDER={provider}" in message
