...
def _get_screenshot_manager_class() -> type[Any] | None:
    candidates = [
        "gsd_browser.screenshot_manager",
        "gsd_browser.screenshots",
...
def test_streaming_env_defaults_and_invalid_input_fallback() -> None:
    ...
    if not hasattr(settings, "streaming_mode") or not hasattr(settings, "streaming_quality"):
        pytest.skip("Streaming settings not yet implemented in gsd_browser.config.Settings")
...
def test_screenshot_manager_get_screenshots_filters() -> None:
    ...
    if cls is None:
        pytest.skip("ScreenshotManager not yet implemented in gsd_browser")
...
def _healthz_payload() -> Mapping[str, Any]:
    candidates = [
        "gsd_browser.log_server",
        "gsd_browser.server",
        "gsd_browser.healthz",
        "gsd_browser.streaming.server",
    ]
    for module_name in candidates:
        try:
            module = importlib.import_module(module_name)
    ...
    pytest.skip(
        "No /healthz handler exposed yet (expected under gsd_browser.streaming.server or similar)"
    )
