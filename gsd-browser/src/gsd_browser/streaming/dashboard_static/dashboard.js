...
function updateControlState(state) {
  const holder = state?.holder_sid ?? null;
  const heldSince = state?.held_since_ts ?? null;
  ...
  setPill(
    $('ctrlStatus'),
    isHeld ? 'control: held' : 'control: free',
    isHeld ? 'pill-warn' : 'pill-muted'
  );

  const connected = Boolean(ctrlSocket?.connected);
  $('btnTake').disabled = !connected || isHeld;
  $('btnRelease').disabled = !connected || !isHeld;
  $('btnPause').disabled = !connected || !isHeld || paused;
  $('btnResume').disabled = !connected || !isHeld || !paused;
}
...
function wireButtons() {
  ...
  $('btnTake').addEventListener('click', () => ctrlSocket?.emit('take_control', {}));
