...
You MUST respond with valid JSON containing an 'action' field that is an ARRAY.
- The 'action' field must ALWAYS be a list/array: `"action": [...]`
- Even a single action must be in an array: `"action": [{"navigate": {"url": "..."}}]`
- The `done` action MUST be inside the action array:
  - ✅ CORRECT: `{"action": [{"done": {"success": true, "text": "..."}}]}`
...
<memory_examples>
"memory": "Visited 2 of 5 target websites. Collected pricing data from Amazon ($39.99) and eBay ($42.00). Still need to check Walmart, Target, and Best Bay for the laptop comparison."
...
The action field must be an array. Examples:
- Single action: `"action": [{{"navigate": {{"url": "https://example.com"}}}}]`
- Multiple actions: `"action": [{{"input_text": {{"index": 10, "text": "query"}}}}, {{"click_element": {{"index": 12}}}}]`
- Done action: `"action": [{{"done": {{"success": true, "text": "Result..."}}}}]`

Action list should NEVER be empty. Even a single action must be in an array.
</output>
