...
def _configure_docket() -> None:
    ...
    fastmcp.settings.docket.name = _env("GSD_TEST_DOCKET_NAME", "gsd-restart-resilience-test")
...
def _identity_for_background_calls() -> Identity:
    return Identity(
...
async def _fake_long_web_eval_agent(*_args: Any, **_kwargs: Any) -> list[TextContent]:
    ...
    deadline = time.time() + float(_env("GSD_TEST_TASK_WORK_S", "2.0"))
...
entry._resolve_identity_for_current_call = _identity_for_background_calls  # type: ignore[assignment]
