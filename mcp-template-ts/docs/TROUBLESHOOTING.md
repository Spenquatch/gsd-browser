...
./scripts/diagnose.sh
```
It prints Node/npm versions, checks `node_modules`, and runs `mcp-template-ts diagnose`.
...
- **Logging format surprises**: Adjust `LOG_LEVEL` / `MCP_TEMPLATE_JSON_LOGS`, or use CLI flags `--log-level`, `--json-logs`, `--text-logs`.
