# gsd-browser-ts (TypeScript)

GSD Browser MCP server (TypeScript). Mirrors the Python server but uses Node.js tooling (npm + TypeScript) while following `../GSD_BROWSER_BLUEPRINT.md` and `tasks.json`.

## Features
- **Node 20+/TypeScript** project with `commander` CLI (`gsd-browser-ts`)
...
```bash
./tools/install.sh
gsd-browser-ts serve --log-level debug
# Upgrades / removal
./tools/upgrade.sh
./tools/uninstall.sh
```
...
```bash
gsd-browser-ts mcp-config --format toml   # or json
```
...
ANOTHER_VAR=1 docker run --rm \
  ...
  gsd-browser-ts:dev serve --json-logs
```

## Logging Controls
- Env vars: `LOG_LEVEL`, `GSD_BROWSER_JSON_LOGS`
